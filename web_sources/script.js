...
const checkLanguageAndSetCallback = () => {
  try {
    ...
    if (activeEditor) {
      const language = activeEditor.session.getMode().$id;
      if (language.includes("c_cpp") || language.includes("python") || language.includes("markdown") || language.includes("biobridge")) {
...
function insertValueIntoEditor(value) {
  const activeTab = document.querySelector(".tab.active");
  if (!activeTab) return;
  const editorId = activeTab.getAttribute("data-editor-id");
  const activeEditor = ace.edit(editorId);

  // Insert the pre-specified value at the current cursor position
  activeEditor.session.insert(activeEditor.getCursorPosition(), value);
}

function transformSequences(text) {
  const patterns = [
    { type: 'DNA', regex: /DNA\s+([\w_]+)\s+"([A-Za-z]+)"/g },
    { type: 'RNA', regex: /RNA\s+([\w_]+)\s+"([A-Za-z]+)"/g },
    { type: 'rRNA', regex: /rRNA\s+([\w_]+)\s+"([A-Za-z]+)"/g },
    { type: 'mRNA', regex: /mRNA\s+([\w_]+)\s+"([A-Za-z]+)"/g },
    { type: 'tRNA', regex: /tRNA\s+([\w_]+)\s+"([A-Za-z]+)"/g }
  ];

  // Loop over each pattern and replace it with the correct format
  patterns.forEach(({ type, regex }) => {
    text = text.replace(regex, (match, sequenceName, sequence) => {
      return `${sequenceName} = ${type}("${sequence}")`;
    });
  });
...
document.getElementById('imageInput').addEventListener('change', function(event) {
  ...
  // Use FileReader to load the image file
  ...
  reader.onload = function(e) {
    ...
    Tesseract.recognize(
      ...
      // Transform the recognized text to replace DNA/RNA patterns
      const transformedText = transformSequences(text);

      // Insert transformed text into the active Ace Editor
      activeEditor.setValue(transformedText, -1); // -1 means move the cursor to the start
    }).catch(err => {
      console.error(err);
      activeEditor.setValue("Error processing image: " + err.message);
    });
  };
